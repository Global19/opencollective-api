...
/**
 * Load a `GuestToken` from it's hash, returns the user and collective associated
 */
...
const createGuestProfile = (email: string, name: string | null): Promise<GuestProfileDetails> => {
  ...
  return sequelize.transaction(async transaction => {
    ...
    // Will throw if email is already taken (which is what we want)
    ...
    if (!user) {
      ...
      // Note that a different Collective will be used for the contribution
    ...
    // this guest profile
    const token = await models.GuestToken.create(
      {
        CollectiveId: collective.id,
        value: guestToken,
      },
...
export const getOrCreateGuestProfile = async ({
  ...
  if (token) {
    ...
    // First time contributing as a guest
    return createGuestProfile(email, name);
  }
};
